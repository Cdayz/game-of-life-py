...
class ImgBackend:
    ...
    def __init__(self, img_path: str,
                 size: StateSize, cell_size: CellSize, line_width: int = 1):
        """
        ...
        self.img_path = pathlib.Path(img_path).absolute()
    ...
    def make_default_structure(self) -> Image:
        ...
        for x in range(0, self.img_w, x_step):
            for y in range(0, self.img_h, y_step):
                image.putpixel((x, y), 180)

        for x in range(0, self.img_h, x_step):
            for y in range(0, self.img_w, y_step):
                image.putpixel((x, y), 180)
    ...
    def to_image(self, state: State):
        ...
        for x in range(self.width):
            for y in range(self.height):
                img = self.put_value(img, (x, y), state)
    ...
    def put_value(self, img: Image, point: Point, state: State) -> Image:
        ...
        """
        xp, yp = point
        value = 0 if state[xp][yp] else 255

        x_start = (self.cell_h + self.line_width) * xp + self.line_width
        y_start = (self.cell_w + self.line_width) * yp + self.line_width


        for x in range(x_start, x_start + self.cell_h):
            for y in range(y_start, y_start + self.cell_w):
                img.putpixel((y,x), value)
